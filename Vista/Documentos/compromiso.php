...
if(intval(($_SESSION['rolide']['idrol']) < 3)||(intval($_SESSION['rolide']['idrol']) > 5)){
	...
	echo '<br><br><br><br>
	       <h4>Este ROL no tiene permisos para realizar cambios en estos registros</h4>';
	exit('<br>
		<br>
		<h4><a href="index.php?c=Menu">REGRESAR AL MENU..-');
}
...
<body>
	...
	<div id="cabecera">
		<ul class="navegador">
			...
			<li><a href="index.php?c=Compromiso&a=nuevo" title="">NUEVO</a></li>
		</ul>
	</div>	
	<br>
	<br>
	<br>
	<form class="datos11" action="index.php?c=Compromiso&a=imprimir" name="admin" method="POST" accept-charset="utf-8">
		<table border="1">
			...
			<tbody>
				<?php foreach($data["compromisos"] as $dato) {
					echo "<tr>";
					echo "<td>".$dato["idcompromiso"]."</td>";
					...
					echo "<td>".$dato["obligaciones"]."</td>";
					echo "<td>".$dato["aportes"]."</td>";
					echo "<td>".$dato["notificacion"]."</td>";
					echo "<td><a href='index.php?c=Compromiso&a=imprimir&id=".$dato["idcompromiso"]."'>IMPRIMIR</a></td>";
					echo "</tr>";
					}?>
				</tbody>
				<tfoot>
